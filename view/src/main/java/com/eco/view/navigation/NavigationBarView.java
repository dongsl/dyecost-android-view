...
public class NavigationBarView extends LinearLayout {
  ...
  private NavigationBarView.OnTabClickListener onTabClickListener;
  private NavigationBarView.OnCenterTabSelectListener onCenterTabClickListener;
  ...
  private void initViews(Context context, AttributeSet attrs) {
    defaultSetting();
    contentView = (RelativeLayout) View.inflate(context, R.layout.layout_nav_container, null);
    ...
    TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.navigation_bar);
  ...
  private void parseStyle(TypedArray attributes) {
    if (attributes != null) {
//要放在前面
      textSizeType = attributes.getInt(R.styleable.navigation_bar_textSizeType, textSizeType);


      msgPointColor = attributes.getColor(R.styleable.navigation_bar_msgPointColor, msgPointColor);
      navigationHeight = attributes.getDimension(R.styleable.navigation_bar_navigationHeight, navigationHeight);
      navigationBackground = attributes.getColor(R.styleable.navigation_bar_navigationBackground, navigationBackground);

      msgPointMoreWidth = attributes.getDimension(R.styleable.navigation_bar_msgPointMoreWidth, msgPointMoreWidth);
      msgPointMoreHeight = attributes.getDimension(R.styleable.navigation_bar_msgPointMoreHeight, msgPointMoreHeight);
      msgPointMoreRadius = attributes.getInt(R.styleable.navigation_bar_msgPointMoreRadius, msgPointMoreRadius);
      tabTextSize = compareTo(getContext(), attributes.getDimension(R.styleable.navigation_bar_tabTextSize, 0), tabTextSize, textSizeType);
      tabTextTop = attributes.getDimension(R.styleable.navigation_bar_tabTextTop, tabTextTop);
      iconSize = (int) attributes.getDimension(R.styleable.navigation_bar_tabIconSize, iconSize);
      hintPointSize = attributes.getDimension(R.styleable.navigation_bar_hintPointSize, hintPointSize);
      msgPointSize = attributes.getDimension(R.styleable.navigation_bar_msgPointSize, msgPointSize);
      hintPointLeft = attributes.getDimension(R.styleable.navigation_bar_hintPointLeft, hintPointLeft);
      msgPointTop = attributes.getDimension(R.styleable.navigation_bar_msgPointTop, -iconSize * 3 / 5);
      hintPointTop = attributes.getDimension(R.styleable.navigation_bar_hintPointTop, hintPointTop);

      msgPointLeft = attributes.getDimension(R.styleable.navigation_bar_msgPointLeft, -iconSize / 2);
      msgPointTextSize = compareTo(getContext(), attributes.getDimension(R.styleable.navigation_bar_msgPointTextSize, 0), msgPointTextSize, textSizeType);
      centerIconSize = attributes.getDimension(R.styleable.navigation_bar_centerIconSize, centerIconSize);
      centerLayoutBottomMargin = attributes.getDimension(R.styleable.navigation_bar_centerLayoutBottomMargin, centerLayoutBottomMargin);

      //加号属性
      centerSelectTextColor = attributes.getColor(R.styleable.navigation_bar_centerSelectTextColor, centerSelectTextColor);
      centerNormalTextColor = attributes.getColor(R.styleable.navigation_bar_centerNormalTextColor, centerNormalTextColor);
      centerTextSize = compareTo(getContext(), attributes.getDimension(R.styleable.navigation_bar_centerTextSize, 0), centerTextSize, textSizeType);
      centerTextTopMargin = attributes.getDimension(R.styleable.navigation_bar_centerTextTopMargin, centerTextTopMargin);
      centerAlignBottom = attributes.getBoolean(R.styleable.navigation_bar_centerAlignBottom, centerAlignBottom);


      lineHeight = attributes.getDimension(R.styleable.navigation_bar_lineHeight, lineHeight);
      lineColor = attributes.getColor(R.styleable.navigation_bar_lineColor, lineColor);


      centerLayoutHeight = attributes.getDimension(R.styleable.navigation_bar_centerLayoutHeight, navigationHeight + lineHeight);

      normalTextColor = attributes.getColor(R.styleable.navigation_bar_tabNormalColor, normalTextColor);
      selectTextColor = attributes.getColor(R.styleable.navigation_bar_tabSelectColor, selectTextColor);

      int type = attributes.getInt(R.styleable.navigation_bar_scaleType, 0);
      if (type == 0) {
      ...
      centerLayoutRule = attributes.getInt(R.styleable.navigation_bar_centerLayoutRule, centerLayoutRule);
      hasPadding = attributes.getBoolean(R.styleable.navigation_bar_hasPadding, hasPadding);

      centerAsFragment = attributes.getBoolean(R.styleable.navigation_bar_centerAsFragment, centerAsFragment);
      attributes.recycle();
    }
  }


  public NavigationBarView setupWithViewPager(@NonNull ViewPager viewPager) {
...
//        }
  ...
  public NavigationBarView setupWithViewPager(@NonNull ViewPager2 viewPager2) {
//        final PagerAdapter adapter = viewPager.getAdapter();
//        if (adapter == null) {
//            throw new IllegalArgumentException("ViewPager does not have a PagerAdapter set");
//        }
...
//若没有设置中间添加的文字字体大小、颜色、则同其他Tab一样
    ...
    switch (mode) {
      case NavigationBarView.NavigationMode.MODE_NORMAL:
        buildNavigation();
        break;
      case NavigationBarView.NavigationMode.MODE_ADD:
        buildAddNavigation();
        break;
      case NavigationBarView.NavigationMode.MODE_ADD_VIEW:
   ...
   * 重置各个参数
   */
  public NavigationBarView defaultSetting() {
    ...
    smoothScroll = false;
    //图标大小
    iconSize = (int) DensityHandler.dip2px(getContext(), 20);

    //提示红点大小
    hintPointSize = DensityHandler.sp2px(getContext(), 6);
    //提示红点距Tab图标右侧的距离
    hintPointLeft = DensityHandler.dip2px(getContext(), -3);
    //提示红点距图标顶部的距离
    hintPointTop = DensityHandler.dip2px(getContext(), -3);

    //消息红点字体大小
    msgPointTextSize = 11;
    //消息红点大小
    msgPointSize = DensityHandler.dip2px(getContext(), 18);
    //消息红点距Tab图标右侧的距离   默认为Tab图标的一半
    msgPointLeft = DensityHandler.dip2px(getContext(), -10);
    //消息红点距图标顶部的距离  默认为Tab图标的一半
    msgPointTop = DensityHandler.dip2px(getContext(), -12);
    //Tab文字距Tab图标的距离
    tabTextTop = DensityHandler.dip2px(getContext(), 2);
    //Tab文字大小
    tabTextSize = 12;
    //未选中Tab字体颜色
    ...
    navigationHeight = DensityHandler.dip2px(getContext(), 60);

    scaleType = ImageView.ScaleType.CENTER_INSIDE;

    canScroll = false;

    centerIconSize = DensityHandler.dip2px(getContext(), 36);
    centerLayoutHeight = navigationHeight;
    centerLayoutBottomMargin = DensityHandler.dip2px(getContext(), 10);
    ...
    //1、普通的Tab 2、中间带按钮（如加号）3、
    mode = NavigationBarView.NavigationMode.MODE_NORMAL;
    ...
    //加号文字距离顶部加号的距离
    centerTextTopMargin = DensityHandler.dip2px(getContext(), 3);
    //是否和其他tab文字底部对齐
    centerAlignBottom = true;

    contentType = NavigationBarView.TabContentType.TYPE_NORMAL;
    centerTextStr = "";
    ...
    //消息红点99+的长度
    msgPointMoreWidth = DensityHandler.dip2px(getContext(), 30);
    //消息红点99+的高度
    msgPointMoreHeight = DensityHandler.dip2px(getContext(), 18);
    //消息红点99+的半径
  ...
  public NavigationBarView centerImageRes(int centerImageRes) {
  ...
  public NavigationBarView centerTextStr(String centerTextStr) {
    this.centerTextStr = centerTextStr;
    return this;
  }

  @IntDef({
      NavigationBarView.TextSizeType.TYPE_DP,
      NavigationBarView.TextSizeType.TYPE_SP,
  })
  ...
  @IntDef({
      NavigationBarView.TabContentType.TYPE_NORMAL,
      NavigationBarView.TabContentType.TYPE_ONLY_IMAGE,
      NavigationBarView.TabContentType.TYPE_ONLY_TEXT,
  })
  @Retention(RetentionPolicy.SOURCE)
  ...
  @IntDef({
      NavigationBarView.NavigationMode.MODE_NORMAL,
      NavigationBarView.NavigationMode.MODE_ADD,
      NavigationBarView.NavigationMode.MODE_ADD_VIEW,
  })
  @Retention(RetentionPolicy.SOURCE)
  ...
  private void buildCommonNavigation() {
    ...
    if (titleItems == null || titleItems.length < 1) {
      contentType = NavigationBarView.TabContentType.TYPE_ONLY_IMAGE;
      tabCount = normalIconItems.length;
    } else if (normalIconItems == null || normalIconItems.length < 1) {
      contentType = NavigationBarView.TabContentType.TYPE_ONLY_TEXT;
  ...
  private void setViewPagerAdapter() {
    if (mViewPager == null) {
      mViewPager = new NavigationViewPager(getContext());
    ...
    if (canScroll) {
      ((NavigationViewPager) getViewPager()).setCanScroll(true);
    } else {
      ((NavigationViewPager) getViewPager()).setCanScroll(false);
    }
  ...
  private void addTabItemView(final int position) {
    View itemView = View.inflate(getContext(), R.layout.layout_nav_tab, null);
    ...
    if (mode == NavigationBarView.NavigationMode.MODE_NORMAL) {
      params.width = getWidth() / tabCount;
    } else if (mode == NavigationBarView.NavigationMode.MODE_ADD) {
      params.width = getWidth() / (tabCount + 1);
    } else if (mode == NavigationBarView.NavigationMode.MODE_ADD_VIEW) {
      params.width = getWidth() / (tabCount + 1);
    }
    ...
    setOvalBg(hintPoint, msgPointColor);
    ...
    msgPointParams.bottomMargin = (int) DensityHandler.dip2px(getContext(), -12);
    ...
    switch (contentType) {
      case NavigationBarView.TabContentType.TYPE_ONLY_IMAGE:
        ...
        break;
      case NavigationBarView.TabContentType.TYPE_ONLY_TEXT:
  ...
  private boolean isAddPage() {
    if (mode == NavigationBarView.NavigationMode.MODE_ADD || mode == NavigationBarView.NavigationMode.MODE_ADD_VIEW)
      return true;
    return false;
  }
  ...
  private void selectCenterTabUI() {
    for (int i = 0; i < tabCount; i++) {
      switch (contentType) {
        case NavigationBarView.TabContentType.TYPE_NORMAL:
  ...
  public void setMsgPointCount(int position, int count) {
    ...
    if (count > 99) {
      setRoundRectBg(getContext(), msgPointView, (int) msgPointMoreRadius, msgPointColor);
      ...
      msgPointView.setLayoutParams(params);
      setOvalBg(msgPointView, msgPointColor);
  ...
  public NavigationBarView centerLayoutHeight(int centerLayoutHeight) {
    this.centerLayoutHeight = DensityHandler.dip2px(getContext(), centerLayoutHeight);
    return this;
  }

  public NavigationBarView scaleType(ImageView.ScaleType scaleType) {
    this.scaleType = scaleType;
    return this;
  }


  public NavigationBarView mode(int mode) {
    this.mode = mode;
    return this;
  }

  public NavigationBarView hasPadding(boolean hasPadding) {
    this.hasPadding = hasPadding;
    return this;
  }

  public NavigationBarView centerIconSize(int centerIconSize) {
    this.centerIconSize = DensityHandler.dip2px(getContext(), centerIconSize);
    return this;
  }


  public NavigationBarView navigationBackground(int navigationBackground) {
    this.navigationBackground = navigationBackground;
    return this;
  }

  public NavigationBarView navigationHeight(int navigationHeight) {
    this.navigationHeight = DensityHandler.dip2px(getContext(), navigationHeight);
    return this;
  }

  public NavigationBarView normalTextColor(int normalTextColor) {
    this.normalTextColor = normalTextColor;
    return this;
  }

  public NavigationBarView selectTextColor(int selectTextColor) {
    this.selectTextColor = selectTextColor;
    return this;
  }

  public NavigationBarView lineHeight(int lineHeight) {
    this.lineHeight = lineHeight;
    return this;
  }

  public NavigationBarView lineColor(int lineColor) {
    this.lineColor = lineColor;
    return this;
  }

  public NavigationBarView tabTextSize(int tabTextSize) {
    this.tabTextSize = tabTextSize;
    return this;
  }

  public NavigationBarView tabTextTop(int tabTextTop) {
    this.tabTextTop = DensityHandler.dip2px(getContext(), tabTextTop);
    return this;
  }

  public NavigationBarView msgPointTextSize(int msgPointTextSize) {
    this.msgPointTextSize = msgPointTextSize;
    return this;
  }

  public NavigationBarView msgPointSize(int msgPointSize) {
    this.msgPointSize = DensityHandler.dip2px(getContext(), msgPointSize);
    return this;
  }

  public NavigationBarView msgPointLeft(int msgPointLeft) {
    this.msgPointLeft = DensityHandler.dip2px(getContext(), msgPointLeft);
    return this;
  }

  public NavigationBarView msgPointTop(int msgPointTop) {
    this.msgPointTop = DensityHandler.dip2px(getContext(), msgPointTop);
    return this;
  }


  public NavigationBarView hintPointSize(int hintPointSize) {
    this.hintPointSize = DensityHandler.dip2px(getContext(), hintPointSize);
    return this;
  }

  public NavigationBarView hintPointLeft(int hintPointLeft) {
    this.hintPointLeft = DensityHandler.dip2px(getContext(), hintPointLeft);
    return this;
  }

  public NavigationBarView hintPointTop(int hintPointTop) {
    this.hintPointTop = DensityHandler.dip2px(getContext(), hintPointTop);
    return this;
  }


  public NavigationBarView titleItems(String[] titleItems) {
    this.titleItems = titleItems;
    return this;
  }

  public NavigationBarView normalIconItems(int[] normalIconItems) {
    this.normalIconItems = normalIconItems;
    return this;
  }

  public NavigationBarView selectIconItems(int[] selectIconItems) {
    this.selectIconItems = selectIconItems;
    return this;
  }

  public NavigationBarView fragmentList(List<Fragment> fragmentList) {
    this.fragmentList = fragmentList;
    return this;
  }

  public NavigationBarView fragmentManager(FragmentManager fragmentManager) {
    this.fragmentManager = fragmentManager;
    return this;
  }

  public NavigationBarView anim(Techniques techniques) {
    if (anim != null) {
      this.anim = techniques;
    } else {
      this.anim = null;
    }
    return this;
  ...
  public NavigationBarView centerLayoutRule(int centerLayoutRule) {
    this.centerLayoutRule = centerLayoutRule;
    return this;
  }

  public NavigationBarView canScroll(boolean canScroll) {
    this.canScroll = canScroll;
    return this;
  }

  public NavigationBarView smoothScroll(boolean smoothScroll) {
    this.smoothScroll = smoothScroll;
    return this;
  }


  public NavigationBarView setOnTabClickListener(NavigationBarView.OnTabClickListener onTabClickListener) {
    this.onTabClickListener = onTabClickListener;
    return this;
  }

  public NavigationBarView setOnCenterTabClickListener(NavigationBarView.OnCenterTabSelectListener onCenterTabClickListener) {
    this.onCenterTabClickListener = onCenterTabClickListener;
    return this;
  }

  public NavigationBarView iconSize(int iconSize) {
    this.iconSize = (int) DensityHandler.dip2px(getContext(), iconSize);
    return this;
  }

  public NavigationBarView centerLayoutBottomMargin(int centerLayoutBottomMargin) {
    this.centerLayoutBottomMargin = DensityHandler.dip2px(getContext(), centerLayoutBottomMargin);
    return this;
  }

  public NavigationBarView centerAsFragment(boolean centerAsFragment) {
    this.centerAsFragment = centerAsFragment;
    return this;
  }

  public NavigationBarView addCustomView(View customAddView) {
    this.customAddView = customAddView;
    return this;
  }

  public NavigationBarView centerTextSize(int centerTextSize) {
    this.centerTextSize = DensityHandler.dip2px(getContext(), centerTextSize);
    return this;
  }

  public NavigationBarView centerNormalTextColor(int centerNormalTextColor) {
    this.centerNormalTextColor = centerNormalTextColor;
    return this;
  }

  public NavigationBarView centerSelectTextColor(int centerSelectTextColor) {
    this.centerSelectTextColor = centerSelectTextColor;
    return this;
  }

  public NavigationBarView centerTextTopMargin(int centerTextTopMargin) {
    this.centerTextTopMargin = DensityHandler.dip2px(getContext(), centerTextTopMargin);
    return this;
  }

  public NavigationBarView centerAlignBottom(boolean centerAlignBottom) {
  ...
  public NavigationBarView.OnTabClickListener getOnTabClickListener() {
  ...
  public NavigationBarView textSizeType(int textSizeType) {
  ...
  public NavigationBarView setMsgPointMoreWidth(float msgPointMoreWidth) {
    this.msgPointMoreWidth = DensityHandler.dip2px(getContext(), msgPointMoreWidth);
    return this;
  }
  ...
  public NavigationBarView setMsgPointMoreHeight(float msgPointMoreHeight) {
    this.msgPointMoreHeight = DensityHandler.dip2px(getContext(), msgPointMoreHeight);
    return this;
  }
  ...
  public NavigationBarView setMsgPointMoreRadius(int msgPointMoreRadius) {
  ...
  public NavigationBarView setMsgPointColor(int msgPointColor) {
    this.msgPointColor = msgPointColor;
    return this;
  }


  private float px2ts(Context context, int isDp, float px) {
    if (isDp == 1) {
      return DensityHandler.px2dip(context, px);
    } else {
      return DensityHandler.px2sp(context, px);
    }
  }

  public float compareTo(Context context, float dimension, float tabTextSize, int textSizeType) {
    if (dimension == 0) {
      return tabTextSize;
    } else {
  ...
  private void setRoundRectBg(Context mContext, View view, int dipRadius, int badgeColor) {
    float radius = DensityHandler.dip2px(mContext, dipRadius);
  ...
  private void setOvalBg(View view, int bgColor) {
